...
parsing_re = re.compile(
    r'Valve (?P<valve_id>\w\w) has flow rate=(?P<flow_rate>\d+); tunnels? leads? to valve(s)? (?P<conns>.+)')
...
def rec(open_valves: FrozenSet[str], cur_valve: str, minute: int, score: int):
    ...
    if cur_valve not in open_valves and valve_score > 0:
        total_open_score = (minute - 1) * valves[cur_valve].flow_rate
        open_score += rec(open_valves | frozenset([cur_valve]),
                          cur_valve, minute - 1, score + total_open_score)
